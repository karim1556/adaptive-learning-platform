  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setStudentDetails((data?.user?.user_metadata as any)?.studentDetails || null)
      } catch {
        // ignore
      }
    })()
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto">
          <div className="p-6 max-w-3xl mx-auto space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6">
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 bg-white/20 rounded-2xl flex items-center justify-center text-white text-3xl font-bold">
                    {(studentDetails?.firstName || user.firstName)?.[0]}{(studentDetails?.lastName || user.lastName)?.[0]}
                  </div>
                  <div className="text-white">
                    <h2 className="text-xl font-semibold">
                      {studentDetails?.firstName || user.firstName} {studentDetails?.lastName || user.lastName}
                    </h2>
                    <p className="text-blue-100">{user.email}</p>
                    {studentDetails?.rollNumber && (
                      <p className="text-blue-200 text-sm">Roll No: {studentDetails.rollNumber}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                    ...
                    <p className="font-semibold text-slate-900 dark:text-white">{studentDetails?.firstName || user.firstName || "—"}</p>
                  </div>
                  <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                    <p className="text-xs text-slate-500 mb-1">Last Name</p>
                    <p className="font-semibold text-slate-900 dark:text-white">{studentDetails?.lastName || user.lastName || "—"}</p>
                  </div>
                  {studentDetails?.rollNumber && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      <p className="text-xs text-slate-500 mb-1">Roll Number / Student ID</p>
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.rollNumber}</p>
                    </div>
                  )}
                  <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                  ...
                  {studentDetails?.grade && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      <p className="text-xs text-slate-500 mb-1">Grade Level</p>
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.grade}</p>
                    </div>
                  )}
                  {studentDetails?.school && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      <p className="text-xs text-slate-500 mb-1">School</p>
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.school}</p>
                    </div>
                  )}
                  ...
                  {studentDetails?.phone && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      ...
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.phone}</p>
                    </div>
                  )}
                  {(studentDetails?.city || studentDetails?.state) && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl md:col-span-2">
                      ...
                      <p className="font-semibold text-slate-900 dark:text-white">
                        {[studentDetails.city, studentDetails.state].filter(Boolean).join(", ")}
                      </p>
                    </div>
                  )}
                </div>
            ...
            {/* Interests & Goals */}
            {studentDetails && (studentDetails.interests?.length > 0 || studentDetails.goals || studentDetails.bio) && (
              <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4">Interests & Goals</h3>
                
                {studentDetails.interests?.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs text-slate-500 mb-2">Subjects of Interest</p>
                    <div className="flex flex-wrap gap-2">
                      {studentDetails.interests.map((interest: string) => (
                        <span
                          key={interest}
                ...
                {studentDetails.goals && (
                  <div className="mb-4">
                    <p className="text-xs text-slate-500 mb-2">Learning Goals</p>
                    <p className="text-slate-900 dark:text-white">{studentDetails.goals}</p>
                  </div>
                )}
                ...
                {studentDetails.bio && (
                  <div>
                    ...
                    <p className="text-slate-900 dark:text-white">{studentDetails.bio}</p>
                  </div>
                )}

                <button
                  onClick={() => router.push("/student/onboarding?edit=true")}
            ...
            {!studentDetails && (
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl border border-blue-200 dark:border-blue-800 p-6">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Complete Your Profile</h3>
                <p className="text-slate-600 dark:text-slate-400 mb-4">
                  Add more details about yourself to personalize your learning experience.
                </p>
                <button
            ...
            {studentDetails?.parentName && (
              <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4">Parent/Guardian</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                    <p className="text-xs text-slate-500 mb-1">Name</p>
                    <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.parentName}</p>
                  </div>
                  {studentDetails.parentEmail && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      <p className="text-xs text-slate-500 mb-1">Email</p>
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.parentEmail}</p>
                    </div>
                  )}
                  {studentDetails.parentPhone && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                      <p className="text-xs text-slate-500 mb-1">Phone</p>
                      <p className="font-semibold text-slate-900 dark:text-white">{studentDetails.parentPhone}</p>
                    </div>
                  )}
                </div>
              </div>
